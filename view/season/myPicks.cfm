						...
						<cfif variables.qryGetUserPicksOfTheWeek.recordCount>
							<cfloop query="variables.qryGetGamesOfTheWeek">
								...
								<cfif qryCheckCurrentUserPick.recordCount>
									<cfif qryGetGamesOfTheWeek.gameDate GT "" AND DateDiff('n',session.today,qryGetGamesOfTheWeek.gameDate) GT 0 AND variables.currentUserID NEQ session.user.userID>	<!--- if the game has not started yet and the loggedIn userID is not the userID in the looped query, then do not show the table row --->
