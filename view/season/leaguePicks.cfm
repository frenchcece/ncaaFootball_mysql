					...
					<tbody>
						<cfif variables.qryGetUserPicks.recordCount>
							...
							<cfif qryCheckCurrentUserPick.recordCount>
								<cfif DateDiff('n',session.today,qryGetGamesOfTheWeek.gameDate) GT 0 AND variables.qryGetLeaguePlayers.userID NEQ session.user.userID>	<!--- if the game has not started yet and the loggedIn userID is not the userID in the looped query, then do not show the table row --->
