<cfcomponent>
	...
	<cffunction name="Init" access="public" output="No">
		...
		<cfargument name="ssl" required="0">
	...
	<cffunction name="list" access="public" output="No">
		...
		<cfloop from="1" to="#arrayLen(Messages)#" step="1" index="index">
			...
			<cfloop from="1" to="#arrayLen(flags)#" step="1" index="i">
				<cfif flags[i].toString() eq flag.SEEN.toString()>
					<cfset querySetCell(qry_Messages,"seen", true)>
				<cfelseif flags[i].toString() eq flag.ANSWERED.toString()>
					<cfset querySetCell(qry_Messages,"answered", true)>
				<cfelseif flags[i].toString() eq flag.DELETED.toString()>
					<cfset querySetCell(qry_Messages,"deleted", true)>
				<cfelseif flags[i].toString() eq flag.DRAFT.toString()>
					<cfset querySetCell(qry_Messages,"draft", true)>
				<cfelseif flags[i].toString() eq flag.FLAGGED.toString()>
					<cfset querySetCell(qry_Messages,"flagged", true)>
				<cfelseif flags[i].toString() eq flag.USER.toString()>
					<cfset querySetCell(qry_Messages,"user", true)>
				<cfelseif flags[i].toString() eq flag.RECENT.toString()>
...
<!---			<cfif findnocase("text/plain", part.getContentType())>
			...
			<cfif IsArray(From)><cfset querySetCell(qry_Messages,"from", arrayToList(From))><cfelse><cfset querySetCell(qry_Messages,"from", From)></cfif>
			<cfset querySetCell(qry_Messages,"messagenumber", objMessage.getMessageNumber())>
			<cfset querySetCell(qry_Messages,"subject", objMessage.getSubject())>
			<cfif IsArray(To)><cfset querySetCell(qry_Messages,"to", arrayToList(To))><cfelse><cfset querySetCell(qry_Messages,"to", To)></cfif>
